...
mapSize = [[64, 64], [32, 32], [16, 16], [8, 8], [4, 4]]
nmsThresh = 0.45
objectThresh = [[0.2, 0.2], [0.25, 0.25], [0.3, 0.3], [0.35, 0.35], [0.4, 0.4]]
    ...
    for index in range(headNum):
        for i in range(mapSize[index][0]):
            for j in range(mapSize[index][1]):
                meshgrid.append(j * strides[index] + strides[index] / 2)
                meshgrid.append(i * strides[index] + strides[index] / 2)
        ...
        for h in range(mapSize[index][0]):
            for w in range(mapSize[index][1]):
                ...
                for cl in range(class_num):
                    ...
                    if cls_val > objectThresh[index][cl]:
                        xmin = (meshgrid[gridIndex + 0] - reg[0 * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w])
                        ymin = (meshgrid[gridIndex + 1] - reg[1 * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w])
                        xmax = (meshgrid[gridIndex + 0] + reg[2 * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w])
                        ymax = (meshgrid[gridIndex + 1] + reg[3 * mapSize[index][0] * mapSize[index][1] + h * mapSize[index][1] + w])
...
if __name__ == '__main__':
    ...
    GenerateMeshgrid()
    img_path = './test.jpg'
    model_path = './fcos.onnx'
    detect(img_path, model_path)